...
namespace nn::fs {
	...
	public:
		...
		bool IsWindowsPathAllowed() { return (value & (1 << 0)) != 0; }
		bool IsRelativePathAllowed() { return (value & (1 << 1)) != 0; }
		bool IsEmptyPathAllowed() { return (value & (1 << 2)) != 0; }
		bool IsMountNameAllowed() { return (value & (1 << 3)) != 0; }
		bool IsBackslashAllowed() { return (value & (1 << 4)) != 0; }
	};

	class PathFormatter {
	public:
		static nn::Result Normalize(char* buffer, uint64_t normalizeBufferLength, const char* path, uint64_t pathLength, nn::fs::PathFlags const&);
		static nn::Result IsNormalized(bool* outIsNormalized, uint64_t* outNormalizedPathLength, const char* path, nn::fs::PathFlags const&);
...
template<size_t N, typename... Ts>
void CreateTest(const char* name, void (*func)(Ts...), const std::tuple<Ts...> testData[]) {
	Buf[0] = '\n';
	BufPos = 1;
...
static constexpr const auto TestData_PathFormatterNormalize_All = make_array(
	...
	std::make_tuple(R"(mount:./aa/bb\cc/dd)", "WRMB"), // This path is considered normalized but the backslashes still normalizes to forward slashes
